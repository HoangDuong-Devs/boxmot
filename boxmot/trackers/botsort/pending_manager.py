...
class PendingManager:
    ...
    def _first_valid_dim(self, *lists, default=512):
        """Find first valid feature dimension form lists."""
        for L in lists:
            if not L: 
                continue
            for f in L:
    ...
    def _long_cost_topk_pending_lost(self, pend_feat_list, lost_tracks, feat_dim):
        """
        Trả về ma trận cost (M x N) với cost_ij = 0.5*(1 - mean_topk_cos(P_i, V_j)),
        trong đó P_i là feat của pending i, V_j là tập vector bank của lost j.
        """
        M, N = len(pend_feat_list), len(lost_tracks)
        if M == 0 or N == 0:
            return np.ones((M, N), dtype=np.float32)

        # Chuẩn hoá pending feats + mask invalid
        ...
        for j, lost in enumerate(lost_tracks):
            V = None
            # Ưu tiên lấy từ bank (nhiều vector)
            if self.vectors_provider is not None:
                try:
                    V = self.vectors_provider(lost)  # (n_j, D) hoặc None
                except Exception:
                    V = None

            # Fallback: centroid proto / local feat
            if V is None or (hasattr(V, "size") and V.size == 0):
                ...
                if self.proto_provider is not None:
                    try:
                        ...
                        v = None
                if v is None:
                    v = getattr(lost, "smooth_feat", None) or getattr(lost, "curr_feat", None)
                if v is not None:
            ...
            if V.ndim == 1:
                V = V[None, :]
            # Chuẩn hoá từng vector trong V
            # V từ Bank vốn đã unit-norm; chỉ re-norm nếu lệch > 1e-2
            ...
            try:
                ...
                col = 0.5 * (1.0 - mean_sims)                 # (M,)
                # mask pending invalid → cost = 1.0
                col = np.where(~p_mask, col, 1.0).astype(np.float32, copy=False)
    ...
    def _stack_and_l2norm_with_mask(self, feature_list, dim):
        """
        Stack features and normalize with invalid mask.
        Returns (matrix, invalid_mask).
        """
        k    = len(feature_list)
        mat  = np.zeros((k, dim), dtype=np.float32)
        mask = np.zeros((k,), dtype=bool)  # True = invalid

        eps = 1e-6  # Consistent epsilon
        ...
        for i, f in enumerate(feature_list):
            ...
            try:
                ...
                if not np.isfinite(norm) or norm < eps:
                    mask[i] = True
                    continue
                # Inly normalize if not already normalized
                if abs(norm - 1.0) > 0.01:
                    mat[i, :] = v / norm
                else:
                    mat[i, :] = v
        ...
        if unmatched_dets:
            try:
                ...
                if with_reid:
                    ...
                    pending_feats, p_mask = self._stack_and_l2norm_with_mask(pend_feat_list, feat_dim)
            ...
            try:
                ...
                for ipend, ilost in pairs:
                    try:
                        ...
                        if st is None:
                            st = {"frames": 0, "last_seen": frame_id}
    ...
    def _batch_iou(self, boxes1, boxes2):
        """Compute batch IoU between two sets of boxes."""
        try:
    ...
    def resolve_id_fragments(self, cand_ids, lost_map, pending_track, frame_id):
        """
        Analyze and resolve ID fragments.
        Returns: (canonical_id, merge_ids).
        """
        try:
            if len(cand_ids) == 1:
                self._log_pl(f"[PM][frame={frame_id}] resolve_id_fragments: single={cand_ids[0]}")
                return cand_ids[0], []
                
            # Feature-based grouping
            feature_groups = self._group_candidates_by_features(cand_ids, lost_map, pending_track)
            self._log_pl(f"[PM][frame={frame_id}] groups(by feat)={feature_groups}")
            
            # Temporal coherence analysis  
            coherent_groups = []
            for group in feature_groups:
                if self._is_temporally_coherent(group, lost_map, frame_id):
                    coherent_groups.append(group)
            
            self._log_pl(f"[PM][frame={frame_id}] coherent_groups={coherent_groups}")

            if len(coherent_groups) == 1 and len(coherent_groups[0]) > 1:
                group = coherent_groups[0]
                canonical_id = self._select_canonical_from_group(group, lost_map, pending_track, frame_id)
                merge_ids = [id for id in group if id != canonical_id]
                self._log_pl(f"[PM][frame={frame_id}] RESOLVE: canonical={canonical_id}, merges={merge_ids}")
                return canonical_id, merge_ids
            
            best_id = self._select_best_individual_candidate(cand_ids, lost_map, pending_track, frame_id)
            self._log_pl(f"[PM][frame={frame_id}] RESOLVE: best_id={best_id} (no full-merge)")
            return best_id, []
        
        except Exception as e:
            print(f"Error resolving ID fragments: {e}")
            return cand_ids[0] if cand_ids else None , []

    def _group_candidates_by_features(self, cand_ids, lost_map, pending_track):
        try:
            if len(cand_ids) <= 1:
                return [[id] for id in cand_ids]

            n = len(cand_ids)
            sim_matrix = np.zeros((n, n), dtype=np.float32)

            for i, id1 in enumerate(cand_ids):
                t1 = lost_map[id1]
                v1 = None
                if self.proto_provider is not None:
                    try:
                        v1, ok1 = self.proto_provider(t1)
                        v1 = v1 if ok1 else None
                    except Exception:
                        v1 = None
                if v1 is None:
                    # fallback an toàn nếu bank trống/lỗi
                    v1 = getattr(t1, "smooth_feat", None) or getattr(t1, "curr_feat", None)

                if v1 is not None:
                    v1 = v1.astype(np.float32, copy=False)
                    v1 /= (np.linalg.norm(v1) + 1e-6)

                for j, id2 in enumerate(cand_ids):
                    if i >= j:
                        continue
                    t2 = lost_map[id2]
                    v2 = None
                    if self.proto_provider is not None:
                        try:
                            v2, ok2 = self.proto_provider(t2)
                            v2 = v2 if ok2 else None
                        except Exception:
                            v2 = None
                    if v2 is None:
                        v2 = getattr(t2, "smooth_feat", None) or getattr(t2, "curr_feat", None)
                    ...
                    if v1 is not None and v2 is not None:
                        ...
                        sim_matrix[i, j] = sim_matrix[j, i] = max(0.0, min(1.0, sim))
                    else:
                        sim_matrix[i, j] = sim_matrix[j, i] = 0.0

            threshold = 0.7
            groups, visited = [], set()
            for i, _id in enumerate(cand_ids):
                if i in visited:
                    continue
                group = [_id]
                visited.add(i)
                for j, other_id in enumerate(cand_ids):
            ...
            print(f"Error grouping candidates: {e}")
            return [[id] for id in cand_ids]

    def _is_temporally_coherent(self, group, lost_map, current_frame):
        """Check if group is temporally coherent."""
        try:
            if len(group) <= 1:
                return True
                
            # Sort by end_frame
            tracks_with_time = [(id, getattr(lost_map[id], "end_frame", 0)) for id in group]
            tracks_with_time.sort(key=lambda x: x[1])
            
            # Check gaps between segments
            max_reasonable_gap = 50  # frames
            for i in range(len(tracks_with_time) - 1):
                _, end1 = tracks_with_time[i]
                _, end2 = tracks_with_time[i + 1]
                if end2 - end1 > max_reasonable_gap:
                    return False
                    
            return True
        except Exception:
            return True # Default to coherent if check fails

    def _select_canonical_from_group(self, group, lost_map, pending_track, current_frame):
        try:
            scores = {}
            pending_feat = (pending_track.smooth_feat if pending_track.smooth_feat is not None
                            else pending_track.curr_feat)
            if pending_feat is not None:
                pending_feat = pending_feat.astype(np.float32, copy=False)
                pending_feat /= (np.linalg.norm(pending_feat) + 1e-6)

            for track_id in group:
                track = lost_map[track_id]
                score = 0.0
                ...
                if self.proto_provider is not None:
                    try:
                        v, ok = self.proto_provider(track)
                        track_feat = v if ok else None
                    except Exception:
                        track_feat = None
                if track_feat is None:
                    track_feat = getattr(track, "smooth_feat", None) or getattr(track, "curr_feat", None)

                if pending_feat is not None and track_feat is not None:
                    track_feat = track_feat.astype(np.float32, copy=False)
                    track_feat /= (np.linalg.norm(track_feat) + 1e-6)
                    feat_sim = float(np.dot(track_feat, pending_feat))
                    feat_sim = max(0.0, min(1.0, feat_sim))
                    score += feat_sim * 0.4

                end_frame = getattr(track, "end_frame", 0)
                gap = max(0, current_frame - end_frame)
                recency = 1.0 / (1.0 + gap / 30.0)
                score += recency * 0.3

                tracklet_length = getattr(track, "tracklet_len", 1)
                quality = min(1.0, tracklet_length / 30.0)
                score += quality * 0.2

                id_bonus = max(0.0, 1.0 - track_id / 1000.0)
                score += id_bonus * 0.1

                scores[track_id] = score

            self._log_pl(f"[PM] canonical scoring: {scores}")
            return max(scores, key=scores.get)
        except Exception as e:
            print(f"Error selecting canonical ID: {e}")
            return group[0]

    def _select_best_individual_candidate(self, cand_ids, lost_map, pending_track, frame_id):
        """Select best candidate when no coherent group exists."""
        return self._select_canonical_from_group(cand_ids, lost_map, pending_track, frame_id)
        
def normalize_feature_standard(feat, eps=1e-6, copy=True):
    """
    Standardized normalization across all components.
    Args:
        feat:input feature vector
        eps: epsilon threshold (consistent 1e-6)
        copy: whether to copy the input
    Returns:
    normalized feature of None if invalid
    """
    if feat is None:
        return None
    
    try:
        feat = np.asarray(feat, dtype=np.float32, copy=copy).ravel()
